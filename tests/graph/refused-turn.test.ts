...
describe("planTurnRollback", () => {
  const ROWS: Array<{
    ...
    (() => {
      const decision = calling("a1", "finish_nudge");
      const produced = [
        nudge("n1"),
        decision,
        new ToolMessage({
      ...
      return {
        name: "a local terminal decision is removable because it has not acted on Chatwoot",
        produced,
        current: produced,
        expected: { action: "remove", ids: ["n1", "a1", "t1"] },
      };
    })(),
    (() => {
