...
class RetiringModel extends BaseChatModel {
  ...
  override bindTools(tools: BindToolsInput[]) {
    const hasFinishNudge = tools.some(
      (candidate) =>
        (candidate as { name?: string } | null)?.name === "finish_nudge",
    );
    if (!hasFinishNudge) return this;
    const self = this;
    let decided = false;
    return {
      async invoke(): Promise<AIMessage> {
        ...
        return new AIMessage({
          ...
          tool_calls: [
            {
              name: "finish_nudge",
              args: { action: "send", message: self.text },
...
const terminalMessages = (messages: BaseMessage[]): string[] =>
  messages.flatMap((message) =>
    ((message as AIMessage).tool_calls ?? [])
      .filter((call) => call.name === "finish_nudge")
...
describe.skipIf(!dbUp)(
  ...
  () => {
    ...
    test("a job retired during generation takes its turn back out of the history", async () => {
      ...
      expect(terminalMessages(after)).not.toContain(
        "Oi, ainda precisa de ajuda?",
      );
    ...
    test("another invoke holding the thread defers the rollback instead of racing it", async () => {
      ...
      const after = await channel(checkpointer, graphThreadId);
      expect(after.some((m) => isNudgeTurn(m))).toBe(true);
      expect(terminalMessages(after)).toContain("Oi, ainda precisa de ajuda?");
    });
