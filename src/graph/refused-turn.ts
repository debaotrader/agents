...
// A tool call is normally the line between a turn that only SAID something and one that DID
// something. The two terminal nudge tools are the deliberate exception: they only record a local
// delivery decision, and the actual Chatwoot write happens later, after every fence. Keeping a
// refused turn merely because it called finish_nudge/skip_reply would make the structured-output
// boundary incompatible with this rollback and put an undelivered turn back in memory.
function actedOnTheWorld(slice: readonly BaseMessage[]): boolean {
  const localDecisionCallIds = new Set<string>();
  for (const message of slice) {
    for (const call of (message as AIMessage).tool_calls ?? []) {
      if (call.name !== "finish_nudge" && call.name !== "skip_reply") {
        return true;
      }
      if (call.id) localDecisionCallIds.add(call.id);
    }
    if (message.getType() === "tool") {
      const callId = (message as BaseMessage & { tool_call_id?: string })
        .tool_call_id;
      if (!callId || !localDecisionCallIds.has(callId)) return true;
    }
  }
  return false;
}
